...
use App\Models\SocialVideo as Media_center;
...
class MediaCenterController extends Controller
{
    ...
    {
        return view('backend.pages.Media_center.create');
    }
...
//             'video' => 'not_regex:<script>',
        ...
        if($request->file('image') or $request->file('video') )
        {
            ...
            if ($request->file('image')) {
                ...
                Image::make($image)->resize(300, 300)->Save(public_path(Media_center::IMAGE_PATH.$name_gen));
            ...
            if($request->file('video')){
                ...
                $path = public_path(Media_center::IMAGE_PATH_VIDEO);
    ...
    public function edit($id)
    {
        ...
        return view('backend.pages.Media_center.edit',compact('Media_center'));
    }
    ...
    public function update(Request $request)
    {
        ...
        if($request->file('image') or $request->file('video') )
        {
            ...
            if ($request->file('image')) {
                ...
                Image::make($image)->resize(300, 300)->Save(public_path(Media_center::IMAGE_PATH.$name_gen));
                $save_url = $name_gen;
                if(public_path(Media_center::IMAGE_PATH.$old_image))
                {
                    unlink(public_path(Media_center::IMAGE_PATH.$old_image));
                }
            ...
            if($request->file('video')){
                ...
                if($old_video)
                {
                    if(file_exists(public_path(Media_center::IMAGE_PATH_VIDEO.$old_video)))
                    {
                        unlink(public_path(Media_center::IMAGE_PATH_VIDEO.$old_video));
                    }
                }
            }
            // end video
